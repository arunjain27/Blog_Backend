...
router.post('/get', Middleware_fun, async (req, res) => {
  try {
    ...
    let finalname="none";
     ...
     console.log(id) 
     const User_blog= await Blog_Schema.find({user:id}); 
      
     console.log(User_blog)
     const reversedData = User_blog.reverse();  
     res.json({userblog:reversedData ,user:finalname})
...
router.post('/allpost', Middleware_fun, async (req, res) => {
  try {
    ...
    let finalname="none";
     if(userid!="none")
     {
       username=await User.findById(userid);
       finalname=username.name;
       console.log(username.name);
       
     }  
    
     const User_blog= await Blog_Schema.find({}); 
     console.log(User_blog)
     const reversedData = User_blog.reverse();     

      res.json({userblog:reversedData , username:finalname})
...
async function improveText(prompt, text) {
  try {
    const response = await axios.post(`https://api.openai.com/v1/text-tools/${MODEL_NAME}/improve`, {
      prompt: prompt,
      text: text,
      model: MODEL_NAME,
    }, {
...
router.post('/newpost', async (req, res) => {
  try {
    const { title, tag, description } = req.body;
    const prompt = "Improve the title blog post: , improve the english , looks more attractive ,creative and professional";
    ...
    res.status(201).json({ title: improvedTitle, tag: improvedTag, description: improvedDescription });
  } catch (error) {
    console.error("Error occurred:", error);
    res.status(500).send("An error occurred");
  }
...
router.post('/generateText', async (req, res) => {
  try {
    ...
    res.status(200).json({ generatedTitle,generatedTag,generatedDescription });
